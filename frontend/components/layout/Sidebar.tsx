...
export function Sidebar({ role = "staff" }: { role?: string }) {
  ...
  useEffect(() => {
    ...
    const fetch = () => getDashboardSummary().then((s) => setPendingCount(s.pending_count ?? 0)).catch(() => {});
    fetch();
          ...
          {visibleItems.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href}
              className={clsx(
                ...
                isActive(href) ? "bg-sprout-green text-white" : "text-white/70 hover:bg-white/10 hover:text-white"
              )}
            >
              <Icon size={18} />
