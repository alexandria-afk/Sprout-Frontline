...
from dependencies import get_current_user, require_manager_or_above, require_admin
...
async def list_leaderboards(
    ...
    return await GamificationService.list_leaderboards(org_id)
...
async def get_leaderboard(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    config, entries = await GamificationService.get_leaderboard_scores(leaderboard_id, org_id)
    if config is None:
...
async def create_leaderboard(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    result = await GamificationService.create_leaderboard(org_id, body.model_dump(exclude_none=False))
    if not result:
...
async def list_badges(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    return await GamificationService.list_badges(org_id)
...
async def my_badges(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    return await GamificationService.list_my_badges(user_id, org_id)
...
async def create_badge(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    result = await GamificationService.create_badge(org_id, body.model_dump(exclude_none=False))
    if not result:
...
async def update_badge(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    result = await GamificationService.update_badge(badge_id, org_id, body.model_dump(exclude_none=False))
    if not result:
...
async def delete_badge(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    await GamificationService.delete_badge(badge_id, org_id)
    return {"success": True}
...
async def award_badge(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    try:
        result = await GamificationService.award_badge(badge_id, body.user_id, org_id, awarded_by)
...
async def my_points(
    ...
    user_id = current_user["sub"]
    result = await GamificationService.get_my_points(user_id)
    return result or {}
...
async def org_leaderboard(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    entries = await GamificationService.get_org_leaderboard(org_id)
...
async def points_summary(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    return await GamificationService.get_points_summary(org_id)
...
async def list_badge_templates(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    return await GamificationService.list_badge_templates(org_id)
...
async def list_leaderboard_templates(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    return await GamificationService.list_leaderboard_templates(org_id)
...
async def seed_templates(
    ...
    org_id = (current_user.get("app_metadata") or {}).get("organisation_id")
    await GamificationService.seed_templates(org_id)
